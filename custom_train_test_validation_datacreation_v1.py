...
Dest_dir = Datadir + "\\dataset\\"
categories = os.listdir(Source_dir)


def divide_datasets(cat_limit, train_size, test_size, validation_size):
    os.mkdir(os.path.join(Dest_dir + '\\train\\'))
    os.mkdir(os.path.join(Dest_dir + '\\test\\'))
    ...
    for category in categories[:cat_limit]:
        ...
        dest_path_train = os.path.join(Dest_dir + '\\train\\' + category)
        dest_path_test = os.path.join(Dest_dir + '\\test\\' + category)
        dest_path_validation = os.path.join(Dest_dir + '\\validation\\' + category)
